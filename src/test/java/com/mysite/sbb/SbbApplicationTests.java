...
class SbbApplicationTests {
	...
	/*
	 * @Test void testJpa() { List<Question> all =
	 * this.questionRepository.findAll(); assertEquals(2, all.size());
	 * 
	 * Question q = all.get(0); assertEquals("sbb가 무엇인가요?", q.getSubject()); }
	 */
	
	/*
	 * @Test void testJpa() { Optional<Question> oq =
	 * this.questionRepository.findById(1); if(oq.isPresent()) { Question q =
	 * oq.get(); assertEquals("sbb가 무엇인가요?", q.getSubject()); } }
	 */
	
	/*
	 * @Test void testJpa() { Question q =
	...
	/*
	 * @Test void testJpa() { List<Question> qList =
	 * this.questionRepository.findBySubjectLike("sbb%"); Question q = qList.get(0);
	 ...
	 * oq.get(); q.setSubject("수정된 제목"); this.questionRepository.save(q); }
	 */
	
	/*
	 * @Test void testJpa() { assertEquals(2, this.questionRepository.count());
	...
	/*
	 * @Test void testJpa() { Optional<Answer> oa =
	 * this.answerRepository.findById(1); assertTrue(oa.isPresent()); Answer a =
	 * oa.get(); assertEquals(2, a.getQuestion().getId()); }
	 */
